...
{
	//1.生成json数组
	//2.判断 /usr/data/mqtt_results.txt 是否存在
	//3.如果存在，判断mqtt_results.txt是否超过十条数据。
	//4.如果超过，删除第一条，再添加一个末端数据
	
	char file_path[50] = "/usr/data/mqtt_results.txt";
	char mqtt_results[2048] = {0};
	FILE *Fd;
	char *mqtt_src = NULL;
	...
	if(access(file_path,F_OK) != 0){
		...
		if(Fd == NULL){
			printf("%s %d: open /usr/data/mqtt_results.txt error!!!!.\n",__func__,__LINE__);
			return -1;
		}
		...
		//读取mqtt数据
		fread(mqtt_results,sizeof(mqtt_results),1,Fd);
		int cnt = 0;
		
		if(strstr(mqtt_results,"{") != NULL){
			pJsonArry = cJSON_Parse(mqtt_results);
			if(NULL == pJsonArry){
				fclose(Fd);
				return -1;
			}
			cnt = cJSON_GetArraySize(pJsonArry);
			//printf("[%s] [%d] cnt:%d \n",__func__,__LINE__,cnt);
			if(cnt > 9){
				//删除第一个数组
				cJSON_DeleteItemFromArray(pJsonArry, 0);
			}
	...
	cJSON_AddStringToObject(pJsonsub, "input",input);      /* 给对象增加内容 */                   
	cJSON_AddStringToObject(pJsonsub, "output",output); 
	
	 mqtt_src = cJSON_PrintUnformatted(pJsonArry);
	...
	memset(mqtt_results,0,sizeof(mqtt_results));
	memcpy(mqtt_results,mqtt_src,strlen(mqtt_src)+1);
	
	fwrite(mqtt_results,sizeof(mqtt_results),1,Fd);
	fclose(Fd);	
...
int get_mqtt_results(char *ASR_COMMAND)
{
	...
	if(access("/usr/data/mqtt_results.txt",F_OK) != 0){
		strcpy(ASR_COMMAND,"暂无数据");
		return -1;			
	} else {
		Fd = fopen(file_path,"r+");	
		if(Fd == NULL){
		...
		if(strstr(mqtt_results,"{") != NULL){
			strcpy(ASR_COMMAND,mqtt_results);
