...
int get_Complete_code(char *t,result_to_mozart_t *sds_results_msg)
{
	...
	char *result_json = NULL;
	result_json = strstr(strMatch,"{");
	int re_num = asr_mcode_json(result_json,&sm,rid,&v,name,&zip,src,number,match_key,sds_results_msg);
	if(re_num == -1)
	{
		printf("%s : parse json error.\n",__func__);
		return -1;//解析json失败
	...
	printf("rid:%s secret_key:%s strClient:%s \n",rid,secret_key,strClient);
	...
	//如果电器类型为空调,保存按键类型和历史命令文件在bngoIR.record
	if(strcmp(match_t,"7") == 0){
		//判断码库是否是含有特殊按键
...
int send_Complete_code(char *path,char *key)
{
	...
	/*************记录历史命令*******************/
	if(strcmp(key,"on") != 0 && strcmp(key,"off") != 0 && strstr(key,"*") == NULL){
		fd = fopen(record,"w+");
		if(fd == NULL){
			printf("%s: open record error!!!!.\n",__func__);
			return OPEN_FILE_ERR;
...
{
	//printf("log: [%s] [%d] path:%s key:%s \n",__func__,__LINE__,path,key);
