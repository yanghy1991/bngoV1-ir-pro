...
int match_to_app_json_other(char *json_str,char *ASR_COMMAND)
{
	...
	char *s3 = cJSON_PrintUnformatted(root);
	memcpy(ASR_COMMAND,s3,strlen(s3)+1);

	if(s3)
		free(s3);

	//printf("[%s] [%d] [s3:%s]\n",__func__,__LINE__,s3);
...
int akeymatch_to_app_json_other(char *json_str,char *ASR_COMMAND)
{
	...
	if(NULL != arrayRs)
	{
		...
		memcpy(ASR_COMMAND,s1,strlen(s1)+1);

		if(s1)
			free(s1);

		
		//printf("[%s] [%d] [s1:%s]\n",__func__,__LINE__,s1);
	...
	fwrite(c_rid->valuestring,128,1,fd);
	fwrite(str_v,10,1,fd);
	fwrite(c_name->valuestring,strlen(c_name->valuestring)+1,1,fd);
	fflush(fd);
	fclose(fd);
	...
	strcat(ASR_COMMAND,"码库获取成功，请最少尝试三个按键");
